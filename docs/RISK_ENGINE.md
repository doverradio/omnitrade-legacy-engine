...
> **Framing correction (architecture reconciliation, 2026-08-06):** This document's framing below (and in §5) still centers "paper accounts" and describes live trading as future/out-of-scope. That framing is stale — live trading is real and in production today (`00_PROJECT_STATE.md`). **The control mechanics described in this document (§2/§3) are, by contrast, still accurate**: this session's audit confirmed the real `evaluate_signal_risk` implementation (`apps/api/app/services/risk/risk_engine.py`) matches this document's evaluation order almost exactly, including the "re-check minimum size after AI scaling" nuance in §3. What this document omits is the campaign/mandate governance layer that now sits alongside it — see `docs/adr/ADR-0011-autonomous-capital-mandate-engine.md` and `docs/adr/ADR-0019-canonical-capital-campaign-governance-layer.md` for that layer, and `RiskEvaluationRequest.campaign_authorized_notional` (a real, load-bearing field not documented here) for how it feeds into the Risk Engine's own minimum-order-rescue logic.

The risk engine is the **mandatory final gate** for every signal before it can become a trade — rules-based strategy signals and AI-adjusted signals alike (see `SYSTEM_ARCHITECTURE.md` §3, `AI_LAYER.md` §3). It can approve, resize, delay, or reject any signal, and can trip account-level or global kill switches. No other component may bypass it. In production today, this remains true for both live-capital campaigns and any paper-only usage — the Risk Engine's authority is not conditional on which mode is running.
...
### 5. Relationship to Live Trading (Historical framing — live trading is now real; see correction in §1)

- The risk engine's control set was designed to remain the same shape when live trading was eventually enabled — this has held: the same 12-step evaluation order and control set described in §2/§3 above governs live campaigns in production today, largely unchanged from its MVP/paper-trading design.
- What the MVP-era bullet below anticipated as a future requirement has since been met, via a different mechanism than originally envisioned here: not a distinct "live account type" with a hard-coded cap, but the campaign/mandate governance layer (`docs/adr/ADR-0011-autonomous-capital-mandate-engine.md`, `docs/adr/ADR-0019-canonical-capital-campaign-governance-layer.md`) that authorizes and scopes live capital deployment *alongside* this Risk Engine, which retains its full, unweakened authority within that scope. See `00_PROJECT_STATE.md` and `00_OPERATIONS_MAP.md` for the current, accurate live-trading posture.
- *(Original MVP-era text, retained for historical record):* "Enabling live trading is explicitly out of MVP scope and requires, at minimum: a distinct 'live' account type, explicit human opt-in per account, and a hard-coded, separately-configured (lower) starting position-size cap. This is a Horizon 2 decision, not an MVP deliverable (see `PROJECT_VISION.md` §5, `MVP_BUILD_PLAN.md`)."

...
describe("LiveCryptoOrderCenter", () => {
  it("renders the live-money gate and blocks submission when the server flag is disabled", async () => {
    ...
    expect(screen.queryByRole("button", { name: "Prepare Confirmation" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Submit Live Order" })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Run Dry Run" })).toBeEnabled();
    expect(screen.getByText("Recent Live Orders")).toBeInTheDocument();
  });

  it("runs a dry run and shows explicit no-submit evidence", async () => {
    installFetchMock();
    render(<LiveCryptoOrderCenter />);
    ...
    expect(await screen.findByText("Dry run status: DRY_RUN_READY")).toBeInTheDocument();
    expect(screen.getAllByText("Dry run completed. No Coinbase order was submitted.").length).toBeGreaterThan(0);
    expect(screen.getByText("Coinbase Create Order called: No")).toBeInTheDocument();
  });
});

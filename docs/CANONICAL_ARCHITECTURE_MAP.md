...
| **Canonical (research/replay walk-forward evaluator)** | `apps/api/app/services/market_state/walk_forward_evaluator.py::run_walk_forward_evaluation` — the canonical harness for measuring the research/replay classifier's state frequencies, persistence, transitions, and forward-return distributions (vs. an unconditional baseline) over chronological OHLCV history, deterministically and without future leakage. Implemented 2026-08-06 as the next step in the same research track. Calls `classify_market_state` for every evaluation point rather than duplicating its logic. **Not consumed by any production path.** See `docs/MARKET_STATE_WALK_FORWARD_EVALUATION.md`. |
...
| **Status** | Live/production for the production canonical implementation. The research/replay classifier and its walk-forward evaluator are built, tested (44 + 40 = 84 unit tests), and available for future consumption (e.g. by an eventual HMM/Bayesian/neural regime candidate needing a baseline to beat), but not yet wired into any broader replay pipeline as a consumer themselves. The two non-canonical candidates are real, tested code, just not authoritative for either canonical use. |

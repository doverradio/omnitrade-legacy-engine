...
See `DECISION_INTELLIGENCE_ENGINE.md`. A permanent foundational engine, standing alongside the Market Intelligence, Strategy Evolution, and Portfolio Intelligence engines. Consumes the output of every decision point — strategy evaluations, AI scoring, and risk engine outcomes — and writes a structured Decision Record for it, whether or not a trade resulted. Every Decision Record is anchored by an immutable **Decision Snapshot** — a point-in-time capture of the exact market context, portfolio state, and strategy/model/config versions that produced the decision (`DECISION_INTELLIGENCE_ENGINE.md` §4a) — so the decision can later be replayed and analyzed exactly, not reconstructed approximately from other tables. It is observational, not participatory: it never influences a decision in real time, it only records after the risk engine has acted, and later enriches that record with outcomes, post-trade review, and AI reflection. Its core subsystems are Decision Records, the **Counterfactual Outcome Ledger (COL)** — a lightweight, continuously-running background process that tracks hypothetical shadow BUY/SELL/WAIT outcomes for every decision and revisits them at fixed horizons, so the platform learns from rejected trades and inaction, not only executed ones (`DECISION_INTELLIGENCE_ENGINE.md` §8) — and the **Decision Quality Engine (DQE)**, which scores how *good* a decision was, independent of whether it happened to be profitable, once counterfactual data is available (`DECISION_INTELLIGENCE_ENGINE.md` §8a). Full architecture, schema, and lifecycle are documented separately; this entry establishes its place in the request flow (see §3). Every one of these subsystems is governed by ADR-0002 (DIE), ADR-0003 (COL), ADR-0004 (Decision Snapshot), and ADR-0007 (DQE) — see `docs/adr/`.

...
The DIE's Decision Quality Engine (DQE), also a core subsystem within it (`DECISION_INTELLIGENCE_ENGINE.md` §8a), anticipates a **Decision Quality Scores** table — one row per scored decision, linked to a Decision Record and the Counterfactual Evaluations it depended on, storing a composite score and its per-dimension breakdown. This table is populated later than the others in this note: a row only exists once COL data has resolved for the relevant decision, so it should never be assumed to exist at the same time as a decision's Decision Record.

No new tables, columns, or migrations are introduced by this note. This section exists so the current schema is read with the DIE's future shape in mind — in particular, `signals`, `model_outputs`, and `risk_events` should continue to be populated completely and consistently, since they are the most likely source data the DIE's Decision Records and the COL's shadow evaluations will be built from. Full column-level schema for the DIE's, COL's, and DQE's tables is deferred to a future revision of this document at implementation time. All of these are governed by ADR-0002, ADR-0003, ADR-0004, and ADR-0007 (see `docs/adr/`).
